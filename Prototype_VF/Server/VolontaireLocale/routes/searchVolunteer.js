...
//______________________________ Rechercher des volontaires de la meme wilaya à reserver__________________________
searchRouter.get('/:id/:startDate/:endDate',async(req,res)=>{
    try{
        ...
        result.splice(0,1);
        let searchconstraint = new constraint();
        searchconstraint.establishmentId=req.params.id;
        searchconstraint.startDate=req.params.startDate;
        searchconstraint.endDate=req.params.endDate;
        ...
        for(i=0;i<wilayaVolunteers.length;i++){
            ...
            console.log(wilayaVolunteers[i].reservationTime.length)
            if(wilayaVolunteers[i].reservationTime.length==0){
                const element = new myvolunteer();
                element.volunteer= wilayaVolunteers[i];
                element.startDate=req.params.startDate;
                element.endDate= req.params.endDate;
                console.log(element)
                result.push(element);
            }else{
                var startprim = searchconstraint.startDate;
                var endprim = searchconstraint.endDate;
                var toadd = true;
...
/*searchRouter.get('/',async(req,res)=>{
    try{
        ...
        const establishmentId = "5ef39715bc7e9104542256bd";//searchconstraint.establishmentId;
        const establishmentvar = await establishment.findById(establishmentId);
        const wilayaVolunteers = await volunteer.aggregate([
            {"$match": {"wilayaCode":establishmentvar.establishmentWilaya}}
            ]);    
        for(i=0;i<wilayaVolunteers.length;i++){
            let start = '2020-06-11T23:00:00.000Z' ;//new constraint(req.body).startDate;
            let end = '2020-07-11T23:00:00.000Z';//new constraint(req.body).endDate;
        // console.log('ser',end);
            if(wilayaVolunteers[i].reservationTime.length==0){
                const element = new myvolunteer();
                element.volunteer= wilayaVolunteers[i];
